...
@Component({
  selector: 'app-report-learners',
  templateUrl: './report-learners.component.html',
  styleUrls: ['./report-learners.component.css']
})
export class ReportLearnersComponent implements OnInit {

  displayColumns = ['name','email','crudBtns'];
  post:any[]  = [];
  ...
  constructor(private http: HttpClient,private fb: FormBuilder,private dataService: ApiService,private router:Router, private dialog: MatDialog) { 
    ...
    this.http.get('http://localhost:8080/angular-admin/php/fetch_data.php').subscribe(data => {
      this.post.push(data);
